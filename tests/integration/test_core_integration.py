...
class TestCoreIntegration:
    ...
    async def test_component_communication(self, setup_components):
        ...
        try:
            self.evolution_engine.add_event_listener("evolution_started", on_evolution_started)
            self.evolution_engine.add_event_listener("evolution_completed", on_evolution_completed)
    ...
    async def test_concurrent_operations(self, setup_components):
        ...
        try:
            ...
            unique_prompts = set(v.prompt for v in best_variants) if best_variants else set()
    ...
    async def test_performance_under_load(self, setup_components):
        ...
        large_engine.sandbox_manager = AsyncMock(test_variant=AsyncMock(side_effect=fast_evaluate))
    ...
    async def test_sustained_metrics_collection(self, setup_components):
        ...
        with patch('datetime.datetime') as mock_datetime:
            ...
            assert collect_duration < 0.01, f"Metric collection took {collect_duration}s"
    ...
    async def test_memory_bounded_operations(self, setup_components):
        ...
        huge_engine.sandbox_manager = AsyncMock(
            test_variant=AsyncMock(return_value=TestResults(
                variant_id=uuid.uuid4(),
                success_rate=0.95,
                avg_response_time=1.0,
                error_count=0,
