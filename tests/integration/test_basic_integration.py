...
from evolution.engine import EvolutionConfig, EvolutionEngine, GeneticAlgorithmStrategy
...
class TestBasicIntegration:
    ...
    async def test_engine_initialization(self):
        ...
        config = EvolutionConfig(
            population_size=10, mutation_rate=0.1, max_generations=2
        )
    ...
    async def test_simple_evolution_cycle(self):
        ...
        try:
            result = await asyncio.wait_for(
                engine.start_evolution_cycle(trigger), timeout=10.0  # 10 second timeout
            )
            print(f"Evolution result: {result}")
            # Result contains cycle_id, trigger, generation, etc.
