...
class MetricsCollector:
    ...
    def collect(self, task_id: str, metrics: Dict[str, Any], agent_id: Optional[str] = None) -> None:
        """
        ...
        try:
            ...
            data_point = MetricsData(
                ...
                metadata={
                    ...
                    if k
                    not in ["response_time", "success", "token_usage", "error_type", "agent_id"]
                },
            )
    ...
    def _cleanup_old_data(self) -> None:
        ...
        for agent_id in list(self.agent_metrics.keys()):
            ...
            self.agent_metrics[agent_id] = deque(
                (dp for dp in self.agent_metrics[agent_id] if dp.timestamp >= cutoff_time),
                maxlen=self.max_data_points
            )
        ...
        if cleaned_count > 0:
            logger.info(f"Cleaned up {cleaned_count} old data points (retention: {self.data_retention_days} days)")
    ...
    async def restore_from_disk(self, path: str) -> None:
        ...
        try:
            ...
            for agent_id, metrics_list in data.get("agent_metrics", {}).items():
                agent_deque = deque(maxlen=self.max_data_points)
    ...
    def register_custom_metric(self, name: str, aggregator: Callable[[List[float]], float], 
    ...
    def detect_anomalies(self, sensitivity: float = 2.0, 
                        window_duration: Optional[timedelta] = None) -> List[Anomaly]:
        """
        ...
        if len(response_times) > 1:
            ...
            for dp in window_data:
                if abs(dp.response_time - mean_rt) > threshold:
                    severity = "high" if abs(dp.response_time - mean_rt) > 3 * stdev_rt else "medium"
        ...
        if len(token_usage) > 1:
            ...
            for dp in window_data:
                if dp.token_usage > 0 and abs(dp.token_usage - mean_tokens) > threshold:
                    severity = "high" if abs(dp.token_usage - mean_tokens) > 3 * stdev_tokens else "medium"
                    anomalies.append(Anomaly(
                        metric_name="token_usage",
                        value=float(dp.token_usage),
                        expected_range=(mean_tokens - threshold, mean_tokens + threshold),
                        severity=severity,
                        timestamp=dp.timestamp,
                        context={"task_id": dp.task_id, "agent_id": dp.agent_id}
                    ))
    ...
    def analyze_metric_correlation(self, metric1: str, metric2: str, 
                                 window_duration: Optional[timedelta] = None) -> CorrelationResult:
        """
        ...
        if len(window_data) < 10:
            return CorrelationResult(
                ...
                correlation_coefficient=0.0, p_value=1.0,
                strength="insufficient_data", direction="none"
            )
        ...
        if not values1 or not values2:
            return CorrelationResult(
                metric1=metric1, metric2=metric2,
                correlation_coefficient=0.0, p_value=1.0,
                strength="invalid_metrics", direction="none"
            )
        ...
        direction = "positive" if corr_coef > 0.1 else "negative" if corr_coef < -0.1 else "none"
        
        # Simple p-value approximation (for production, use scipy.stats)
        n = len(values1)
        t_stat = corr_coef * ((n - 2) / (1 - corr_coef**2))**0.5 if abs(corr_coef) < 1 else float('inf')
        p_value = max(0.001, min(0.999, 2 * (1 - abs(t_stat) / (abs(t_stat) + n))))
        
        return CorrelationResult(
            metric1=metric1, metric2=metric2,
            correlation_coefficient=corr_coef, p_value=p_value,
            strength=strength, direction=direction
        )

    def export_to_csv(self, path: str, window_duration: Optional[timedelta] = None) -> None:
        ...
        with open(path, 'w', newline='') as csvfile:
            fieldnames = ['timestamp', 'task_id', 'agent_id', 'response_time', 
                         'success', 'token_usage', 'error_type']
    ...
    def export_to_json(self, path: str, window_duration: Optional[timedelta] = None) -> Dict[str, Any]:
    ...
    def subscribe_to_stream(self, callback: Callable[[MetricsData], None]) -> None:
        ...
        logger.info(f"Added streaming subscriber, total: {len(self._streaming_subscribers)}")
    ...
    def unsubscribe_from_stream(self, callback: Callable[[MetricsData], None]) -> None:
        ...
        logger.info(f"Removed streaming subscriber, total: {len(self._streaming_subscribers)}")

    def get_agent_metrics(self, agent_id: str, window_duration: Optional[timedelta] = None) -> MetricWindow:
        """
        ...
        """
        return self._get_agent_window_metrics(agent_id, window_duration or self.window_1hr)

    def get_custom_metric_value(self, metric_name: str, window_duration: Optional[timedelta] = None) -> Optional[float]:
        """
    ...
    def _get_agent_window_metrics(self, agent_id: str, window_duration: timedelta) -> MetricWindow:
        ...
        return MetricWindow(
            start_time=start_time, end_time=end_time,
            total_requests=total_requests, success_count=success_count,
            avg_response_time=total_response_time / total_requests,
            total_tokens=total_tokens, error_counts=dict(error_counts)
        )

    def _get_agent_breakdown(self, window_duration: timedelta) -> Dict[str, Dict[str, Any]]:
